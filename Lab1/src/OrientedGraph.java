...
public class OrientedGraph 
{
    		...
    		for (int i = 0; i < addNow; i++)
    			for (int j = 0; j < addNow; j++)
    				for (int k = 0; k < addNow; k++)
    					bridge[i][j][k] = "0";
    		for (int i = 0; i < addNow; i++)
    			map.put(mVex[i].ivex, i);			// Accelerate search process by using advanced data structure; ____ Liu and Li
    		String word1;
    ...
    public String randomWalk()							// Can't be used be map is completed (function generateBridge)
    {
    		String result = "";
	    	...
	    	while (true)
	    	{
		    	...
		    	if (numOut == 0) return result;  // no out edge, end
		    ...
		    j = map.get(tmp.ivex);
		    	...
		    	else return result;  // visited edge, end
		    	i = j;
		    	tmp = mVex[i];
		    	tmp2 = tmp;
	    	}
    ...
    public String dijkstra(String word1) 
    {
        ...
        find[vs] = true;
        for (int i = 1; i < addNow; i++)     //循环扩展n-1次 
        {
        		min = Integer.MAX_VALUE;
            for (int j = 0; j < addNow; j++)    //寻找未被扩展的权值最小的顶点 
            {
            ...
            find[nearest] = true;
            for(int k = 0; k < addNow; k++)   //更新dist数组的值和路径的值 
            {
        ...
        /*for (int i = 0; i < addNow; i++)
        {
        		if (mindist[i] != Integer.MAX_VALUE/2)
        ...
        for (int i = 0; i < addNow; i++)			// 对每一个点寻找路径
        {
        		if (i != (int) map.get(word1))		// 起点不为终点
        		{
		        	for (int k = 0; k < addNow; k++)
		        			vistedIf[k] = 0;
    ...
    public String dijkstra(String word1, String word2) 
    {
        ...
        Integer[] vistedIf = new Integer[addNow];
    		...
    		if (word2Num != (int) map.get(word1))		// 起点不为终点
    		{
	        	for (int k = 0; k < addNow; k++)
	        			vistedIf[k] = 0;
        		path.push((int) map.get(word1));
    ...
    {
        OrientedGraph test1 = new OrientedGraph(5);
        test1.add("i", "write");
